...
2. 在C语言中，任何非0值都是真的，0值都是假；在C语言中使用1表示真值，使用0表示非真值
...
3. 当signed int和unsigned int一起计算时，signed int会被转成unsigned int，会导致严重的bug
...
4. 在c语言中，unsigned数据溢出了会丢掉溢出的位，而signed数据溢出的结果是未定义
