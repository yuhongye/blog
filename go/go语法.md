...
与大多数现代高级语言一样，Go 语言会给声明但未赋值的变量默认赋零值。
...
2. 整型: 
   ...
   2. byte 是uint8的别名，可以和uint8互用，大家一般使用byte; 
   3. int和uint比较特殊，它根据cpu的位数来决定是64位还是32位的，在没有显式转换的情况下不能和int32/int64/uint32/uint64进行赋值、比较和其他数据运算。优先使用int(why?)。
3. 浮点数: float32, float64，优先使用 float64, 浮点数的字面量的默认类型也是 float64。
4. string是__基本类型__，支持`==, !=, >, >=, <, <=`，它的零值是空字符串，string有其复杂性：
   1. 底层存储的是字节 `byte`， 内置的函数 `len`, 派生(slice)都是从字节的角度来看
   2. 用户看到的是字符 `rune`, 字符占用的字节数要根据编码而定


##### 特殊类型

1. `byte` 是 `uint8` 的别名
2. `rune`是 `int32` 的别名，在使用字符建议使用 `rune`, 这样可以增加代码的可读性，明确表达程序的逻辑
...
Go 语言的设计从清晰表达意图和可读性出发，Go语言认为代码的通俗易懂优于简短，因此 Go 不允许变量之间进行自动类型转换，当变量之间类型不一致时，就必须进行显式地类型转换，其他和 Java 类似。

```
var i int = 10
var f float64 = 30.2
// 必须要对 i 进行显式类型转换才能参与计算
val sum float64 = float64(i) + f
```
...
还可以使用`x := 10`，不过这种类型只能在`function`内部使用，定义包变量时不可以, `:=` 可以给已有变量赋值，造成变量覆盖问题。

在这么多种赋值方式中，只需要选择最能表达意图、最有可读性的方式即可。

##### 常量

常量表达式的值在编译器确定，而不是在运行期，因此所有常量表达式的结果都是基础类型(Java可以是任意类型), Go 没有提供一种方法来指定运行时计算的值是不可变的。
...
1. 所有字符在 Go 语言中都是合法的变量名
2. Go 推荐使用驼峰法命名
3. 包级变量通过首字符大小来判断包外的可见性，这特性也够傻逼的，已经引入这么多复杂度了，害怕引入private, public 这俩关键字吗?
