...
1. bool 类型
2. 整型: int8, int16, int32, int64; uint8, uint16, uint32, uint64; byte 是uint8的别名，可以和uint8互用，大家一般使用byte; int和uint比较特殊，它根据cpu的位数来决定是64位还是32位的，在没有显式转换的情况下不能和int32/int64/uint32/uint64进行赋值、比较和其他数据运算。优先使用int。
...
常量表达式的值在编译器确定，而不是在运行期，因此所有常量表达式的结果都是基础类型。
